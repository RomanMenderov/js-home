import { addForm } from "./homework_7";
...
describe("addForm", () => {
  ...
  it("shows button if value is not empty", () => {
    ...
    expect(el.querySelector("button").innerText).toEqual("Add value");
  ...
  it("adds new li on click by button", () => {
    addForm(el);
    const input = el.querySelector("input");
    ...
    expect(el.querySelectorAll("li")[3].innerHTML).toBe(text);
  });
  ...
  it("has limited number of paragraphs", () => {
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      if (i > 1) {
        ...
        expect(el.querySelectorAll("li")[4].innerHTML).toBe(text);
      }
    }
  });
